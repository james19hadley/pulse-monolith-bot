...
# Replace the handler
old_handler = """@router.callback_query(F.data == "settings_test_report")
...
new_handler = """@router.callback_query(F.data == "settings_test_report")
async def cq_test_report(callback: CallbackQuery):
    from src.db.repo import SessionLocal
    from src.bot.handlers.utils import get_or_create_user
...
if old_handler in text:
    ...
    # Just in case
    text = re.sub(r'@router\.callback_query\(F\.data == "settings_test_report"\)[\s\S]*?await callback\.answer\(\)', new_handler, text)
