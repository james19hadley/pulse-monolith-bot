...
- [x] Write logic to alter the database schema safely (or use a simple script for SQLite).
...
- [x] Allow the user to update their Catalyst settings via chat command (e.g. `/settings catalyst 30`).
...
- [x] Extract hardcoded strings from `src/bot/handlers.py` and `jobs.py`.
- [x] Move them to `src/bot/views.py`.
- [x] (Optional) Add a simple toggle for Persona Styles if requested.
