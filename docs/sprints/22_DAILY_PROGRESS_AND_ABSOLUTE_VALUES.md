...
## 📋 Tasks
### UI & Core Data
- [ ] Update project stats retrieval to calculate and return "Progress today" and "Last active" metrics.
- [ ] Modify the UI presentation in inline menus or messages to cleanly display these new metrics.

### AI & Router Extraction
- [ ] Update `prompts.py` (Intent Router and/or Extractors) to deduce whether the user wants to add an incremental value or set an absolute value (`set_to_value=X` vs `add_value=Y`).
- [ ] Implement logic in handlers to calculate the delta (`delta = X - current_value`) when an absolute value is provided, ensuring `TimeLog` records reflect the accurate change without overwriting state redundantly.
