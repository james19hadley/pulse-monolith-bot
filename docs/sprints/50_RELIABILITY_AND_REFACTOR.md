...
- [x] **Task 2:** Define Pydantic models / Data Schemas for every extraction. Examples:
  - `LogWorkSchema(project_id: int, duration_minutes: int)`
  - `AddTasksSchema(tasks: list[TaskModel])`
  - *(Implemented in `src/ai/schemas.py`)*
...
- [x] **Task 5:** Audit the codebase for files exceeding ~300 lines. Focus especially on core modules and handler files.
- [x] **Task 6:** Split large files into logical sub-modules (e.g., separating database queries, UI text generation, and business logic into different cohesive blocks).
  - *Refactored `jobs.py`, `sessions.py`, `core.py`, `intent_entities.py`, and project actions.*
