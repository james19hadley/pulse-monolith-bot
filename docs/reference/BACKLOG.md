...
### 1. Voice-to-Intent Pipeline 🎙️
- **Concept:** Allow users to send voice messages instead of typing commands (e.g., "log 1 hour on monolith bot").
- **Implementation Paths:** 
  - *Option A:* Send audio directly to an LLM provider that supports multi-modal audio inputs.
  - *Option B (Engineering Challenge):* Run local speech-to-text (STT) like Whisper on our server, then feed the transcribed text into the standard Intent Router.
  - *Action:* Experiment with both to compare speed, quality, and server load.

### 2. Weekly & Monthly Gamification Digests 📊
- **Concept:** Provide a "helicopter view" of productivity beyond the daily report.
- **Features:** 
  - Weekly and monthly aggregated summaries generated by the Persona AI.
  - Track "Streaks" (e.g., hitting a project target 5 days in a row).
  - Pacing metrics to show if on track to meet long-term project deadlines.

### 3. Model Context Protocol (MCP) Integration 🔌
- **Concept:** Instead of manually gathering user context (projects, tasks, memory) before every AI call, integrate an MCP server that the LLM can query on-demand.
- **Benefits:**
  - Reduces token usage for simple queries.
  - Allows the AI to "choose" what data it needs to see.
  - Standardizes tool calling across different models.
- **Note:** Proceed with caution as some implementations are considered bloated. Evaluate lightweight alternatives first.
